...
public class CarouselDao extends AbstractDao<News> implements IDao {

	private static final Logger LOG = Logger.getLogger(CarouselDao.class);
	private Parameters params;
	private TableData tableData;
	private MySQL mysql;

	public CarouselDao() {
		params = new Parameters(DATABASE, CAROUSEL_TABLE);
		tableData = Jdcb.getTableData(DATABASE, CAROUSEL_TABLE);
	...
	public String update(News obj) {
		String sql = "UPDATE " + DATABASE + "." + CAROUSEL_TABLE
				+ " SET title='" + obj.getTitle() + "', lead='" + obj.getLead()
				+ "', source='" + obj.getSource() + "', text='" + obj.getText()
				+ "', imgid=" + obj.getImgid() + ", time='"
				+ Transfer.getTimeString() + "' WHERE id=" + obj.getId();
		LOG.debug("Sql: " + sql);
